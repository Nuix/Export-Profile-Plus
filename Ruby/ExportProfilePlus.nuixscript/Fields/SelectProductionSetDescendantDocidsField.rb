class SelectProductionSetDescendantDocidsField < CustomFieldBase
	def name
		return "Select Production Set Descendant DOCIDs"
	end

	def tool_tip
		return "Returns a delimited list DOCIDs for specific selected production sets a given top level item's descendants are a member of."
	end
	...
	def docids_for_item(item)
		...
		if NuixConnection.getCurrentNuixVersion.isLessThan("7.4")
			result = descendants.map {|descendant| @docid_lookup[descendant.getGuid]}
		else
			prod_set_items = descendants.map{|descendant| descendant.getProductionSetItems.toArray}.flatten
		...
		if result.size > 0
			return result.sort.uniq.join(CustomFieldBase.delimiter)
		else
			return ""
		end
	end
